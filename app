...
const WorkoutTracker = () => {
  ...
  const getTypeColor = (type) => {
    switch(type) {
      case 'DS': return 'text-orange-600 bg-orange-100';
      case 'SP': return 'text-purple-600 bg-purple-100';
  ...
  const getTypeLabel = (type) => {
    switch(type) {
      case 'DS': return 'DROP SET';
      case 'SP': return 'ESPECIAL';
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-4">
          ...
          <div className="mb-3">
            ...
            <div className="w-full bg-gray-200 rounded-full h-3">
              <div 
                className={`h-3 rounded-full transition-all duration-300 ${workouts[currentWorkout].color}`}
          ...
          <div className="p-4">
            {workouts[currentWorkout].exercises.map((exercise, index) => (
              <div key={exercise.id} className="flex items-center gap-3 p-3 border-b border-gray-100 last:border-b-0">
                <button
                  onClick={() => toggleExercise(currentWorkout, exercise.id)}
                ...
                <span className={`px-2 py-1 rounded text-xs font-bold ${getTypeColor(exercise.type)}`}>
            ...
            >
              {currentProgress === 100 ? '🔥 FINALIZAR TREINO 🔥' : `Faltam ${workouts[currentWorkout].exercises.length - Math.round((currentProgress/100) * workouts[currentWorkout].exercises.length)} exercícios`}
            </button>
          </div>
        </div>
              ...
              {workoutHistory.map((entry, index) => (
                <div key={index} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-gray-800 font-medium text-sm">{workouts[entry.workout].name.split(' - ')[1]}</span>
                  </div>
